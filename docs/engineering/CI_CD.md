...
Automatic ingestion is required for pending migrations, importer source/script changes, the current target's ingestion wrapper changes, the importer's package manifest, or any lockfile change (conservative dependency handling). A change only to `ingest-production.sh` does not refresh Cloud DEV, and vice versa; Pi-only wrapper changes trigger neither cloud ingestion. The databases and credentials remain independent: deployment never copies the Pi database. Frontend, documentation and test-only changes skip ingestion. Migration changes are conservatively treated as requiring a refresh even when they only change an index. Manual `force_ingest` handles upstream changes without a code change. There is no unsafe “skip required ingestion” switch.
...
In the user-token form, select **Account → Workers Scripts → Edit** and **Account → Workers Builds Configuration → Read**, with resources restricted to the approved account. The Builds API reference calls the latter permission **Workers CI Read**; the dashboard labels it **Workers Builds Configuration**.

For GitHub-hosted runners, use the project's **Session pooler** connection from Supabase's **Connect** panel on port **5432**, preserving its exact host and `postgres.<projectRef>` username. Replace the password placeholder with the database password, percent-encode reserved characters, and include `sslmode=require`. Supabase's default direct endpoint is IPv6-only; [GitHub Actions needs an IPv4-compatible connection](https://supabase.com/docs/guides/troubleshooting/supabase--your-network-ipv4-and-ipv6-compatibility-cHe3BP). An IPv4-enabled direct endpoint is also accepted. Transaction mode on port 6543 is rejected because ingestion needs a session advisory lock. Do not switch an existing session connection blindly when an opaque database error occurs.
...
- Release SQL queries parse the validated connection URL into `PGHOST`, `PGPORT`, `PGDATABASE`, `PGUSER`, `PGPASSWORD` and `PGSSLMODE`; `PGDATABASE` must contain only the database name. Passing the full URL through `PGDATABASE` makes libpq use its default host/socket instead of the intended server. Passwords are percent-decoded once and kept out of command arguments. PR CI tests the same query helper with real `psql` against the disposable database, including rejection of writes and preservation of the statement timeout. See [PostgreSQL environment variables](https://www.postgresql.org/docs/16/libpq-envars.html).
- Read-only database audit failures report a static query stage (`migration-table`, `migration-history`, or `reference-integrity`), connection mode, and an allowlisted error category. Client diagnostics run in the C locale; raw stderr, exception messages, usernames, hosts and credentials are never printed. Categories indicate what the client reported, not a verified root cause: `authentication` calls for checking credentials, `pooler-identity` the Connect panel's host/user pair, `network-unreachable` routing/IP compatibility, `dns-resolution` the endpoint, `tls` certificate configuration, `connection-options` client/pooler compatibility, and `access-denied` permissions/network restrictions. Unknown failures remain `unclassified`; never expose raw errors just to investigate them. All failures still stop delivery.
