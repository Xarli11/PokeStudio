...
Status: `pokelab` Worker created (placeholder, no bindings/Git integration, prechecks passed); repo targets switched to it. The old `pokestudio` Worker is untouched and stays alive for a grace period; the `pokestudio.carlosgt2001.workers.dev` URL is no longer a pipeline target.
