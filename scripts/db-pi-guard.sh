...
# POKESTUDIO_PI_DB_URL. Exists so an accidental localhost/other-host URL
...
require_pi_db_url() {
  if [ -z "${POKESTUDIO_PI_DB_URL:-}" ]; then
    echo "error: POKESTUDIO_PI_DB_URL is not set." >&2
  ...
  local url="$POKESTUDIO_PI_DB_URL"
  ...
  if [ "$host" != "$PI_DB_HOST" ] || [ "$port" != "$PI_DB_PORT" ] || [ "$db" != "$PI_DB_NAME" ]; then
    echo "error: POKESTUDIO_PI_DB_URL does not point at the canonical Raspberry Pi database." >&2
...
pi_db_url_no_ssl() {
  case "$POKESTUDIO_PI_DB_URL" in
    *\?*) echo "${POKESTUDIO_PI_DB_URL}&sslmode=disable" ;;
    *) echo "${POKESTUDIO_PI_DB_URL}?sslmode=disable" ;;
  esac
}
